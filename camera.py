...
class Camera:
    ...
    def __init__(self, resolution=(1640, 1232), format="RGB888"):
        ...
        self.upper_dark = np.array([180, 255, 70])
        self.lower_light = np.array([0, 0, 180])
        self.upper_light = np.array([180, 40, 255])
    ...
    def coordinate(self, image):
        frame_blurred = cv2.GaussianBlur(image, (5, 5), 0)

        frame_hsv = cv2.cvtColor(frame_blurred, cv2.COLOR_RGB2HSV)
        frame_gray = cv2.cvtColor(frame_blurred, cv2.COLOR_RGB2GRAY)

        mask_dark = cv2.inRange(frame_hsv, self.lower_dark, self.upper_dark)
        mask_light = cv2.inRange(frame_hsv, self.lower_light, self.upper_light)
        mask_gray = cv2.threshold(frame_gray, self.gray_threshold, 255, cv2.THRESH_BINARY_INV)[1]
        mask_combined = cv2.bitwise_or(mask_dark, mask_light)
        ...
        if valid_detections:
            best_center = max(valid_detections, key=lambda c: c[0])[1]
